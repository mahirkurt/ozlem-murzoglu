...
function cleanClassName(fileName) {
  // İlk karakteri rakamsa "Doc" ekle
...
async function createComponentFiles(category, fileName, htmlContent) {
  const componentName = cleanComponentName(fileName);
  const className = cleanClassName(fileName);
  ...
  <div class="resource-content">
    <div class="container">
      ...
      <div class="action-buttons">
        ...
        <a href="/documents/${encodeURIComponent(category)}/${encodeURIComponent(fileName)}" download class="btn-primary">
...
function processHtmlForMD3(html) {
  // Clean up Word-specific HTML and truncate if too long
  let processed = html
    .replace(/<o:p><\/o:p>/g, '')
    .replace(/<o:p>&nbsp;<\/o:p>/g, '')
    .replace(/style="[^"]*"/g, '')
  ...
  // If content is too long (likely base64 images), truncate and show sample
  if (processed.length > 50000) {
    processed = `
      <div class="notice">
        <h2>Döküman İçeriği</h2>
        <p>Bu döküman orijinal formatında görüntüler ve detaylı içerik barındırmaktadır.</p>
        <p>Tam içeriğe erişmek için lütfen yukarıdaki "İndir" butonunu kullanarak orijinal dosyayı indirin.</p>
      </div>
      ${processed.substring(0, 5000)}
      <div class="notice">
        <p><em>Dökümanın tamamı için lütfen orijinal dosyayı indirin...</em></p>
      </div>
    `;
  }
...
async function processSampleDocuments() {
  ...
  for (const sample of sampleFiles) {
    ...
    try {
      ...
      if (htmlContent) {
        ...
        console.log(`✓ Processed: ${sample.file} -> ${component.className}`);
      }
    } catch (error) {
  ...
  components.forEach(c => {
    console.log(`  - ${c.category}/${c.componentName} (${c.className})`);
  });
}
